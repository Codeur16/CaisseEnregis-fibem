...
const languages = [
  { code: 'fr', nameKey: 'francais', flag: '🇫🇷', img: '/flags/france.png' },
  { code: 'en', nameKey: 'english', flag: '🇬🇧', img: '/flags/usa.png' },
  // { code: 'es', nameKey: 'espanol', flag: '🇪🇸', img: '/flags/espagne.png' },
  // { code: 'de', nameKey: 'deutsch', flag: '🇩🇪', img: '/flags/germany.png' },
  // { code: 'it', nameKey: 'italiano', flag: '🇮🇹', img: '/flags/italie.png' },
  // { code: 'pt', nameKey: 'portugues', flag: '🇵🇹', img: '/flags/portugais.png' },
  // { code: 'nl', nameKey: 'nederlands', flag: '🇳🇱', img: '/flags/pays-bas.png' },
  // { code: 'pl', nameKey: 'polski', flag: '🇵🇱', img: '/flags/pologne.png' },
  // { code: 'ru', nameKey: 'russkiy', flag: '🇷🇺', img: '/flags/russie.png' },
  // { code: 'ar', nameKey: 'arabic', flag: '🇸🇦', img: '/flags/arabic.png' }
]
...
export default function Header() {
  ...
  const tLanguages = useScopedI18n('languages')
                                    ...
                                    <NavigationMenuLink asChild>
                                      <Link
                                        ...
                                        className={cn(
                                          ...
                                          pathname === `/${currentLocale}${subItem.href}` || pathname?.startsWith(`/${currentLocale}${subItem.href}/`) ? "text-white bg-fibem-accent shadow-md" : ""
                                        )}
                                      >
                                        <ChevronRight className="w-4 h-4 text-fibem-primary mr-2 shrink-0" />
                                        <span className="text-sm font-medium">{t(`menu.${subItem.label}` as any)}</span>
                                      </Link>
                                    </NavigationMenuLink>
                                  )}
                                </li>
                        ...
                        <NavigationMenuLink asChild>
                          <Link
                            ...
                            className={cn(
                              ...
                              (item.href === '/' ? pathname === `/${currentLocale}` || pathname === `/${currentLocale}/` : pathname === `/${currentLocale}${item.href}` || pathname?.startsWith(`/${currentLocale}${item.href}/`)) ? "text-white bg-fibem-accent shadow-lg" : ""
                            )}
                          >
                            {t(`menu.${item.label}` as any)}
                          </Link>
                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    {currentLang?.img && (
                      <img
                        src={currentLang.img}
                        alt={currentLang.nameKey}
                        width={20}
                        height={20}
                        style={{ display: 'inline', verticalAlign: 'middle' }}
                      />
                    )}
                    <span className="hidden sm:inline text-xs font-medium uppercase">{currentLocale}</span>
                    <ChevronDown className="w-3 h-3 shrink-0 opacity-70" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-48 bg-fibem-secondary border-white/10">
                  <DropdownMenuLabel className="text-white/80">{t('chooseLanguage')}</DropdownMenuLabel>
                  <DropdownMenuSeparator className="bg-white/10" />
                  {languages.map((lang) => (
                    <DropdownMenuItem
                      key={lang.code}
                    ...
                    >
                      ...
                      <span className="text-sm">{tLanguages(lang.nameKey as any)}</span>
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
                  ...
                  {session ? (
                    <>
                      <div className="px-2 py-1.5">
                        <p className="text-sm font-medium text-white">{session.user?.first_name || t('account')}</p>
                        <p className="text-xs text-white/60 truncate">{session.user?.email}</p>
                      </div>
                      <DropdownMenuSeparator className="bg-white/10" />
                      <DropdownMenuItem asChild>
                        <Link href={`/${currentLocale}/dashboard`} className="cursor-pointer flex items-center gap-2 text-white/90 hover:text-white">
                          <LayoutDashboard className="w-4 h-4" />
                          {t('dashboard')}
                        </Link>
                      </DropdownMenuItem>
                      <DropdownMenuItem asChild>
                        <Link href={`/${currentLocale}/dashboard/profil`} className="cursor-pointer flex items-center gap-2 text-white/90 hover:text-white">
                          <User className="w-4 h-4" />
                          {t('profile')}
                        </Link>
                      </DropdownMenuItem>
                      <DropdownMenuItem asChild>
                        <Link href={`/${currentLocale}/dashboard/parametres`} className="cursor-pointer flex items-center gap-2 text-white/90 hover:text-white">
                    ...
                    <>
                      <DropdownMenuLabel className="text-white">{t('login')}</DropdownMenuLabel>
                      <DropdownMenuSeparator className="bg-white/10" />
                      <DropdownMenuItem asChild>
                ...
                <div key={item.label}>
                  <Link
                    ...
                    className={cn(
                      ...
                      (item.href === '/' ? pathname === `/${currentLocale}` || pathname === `/${currentLocale}/` : pathname === `/${currentLocale}${item.href}` || pathname?.startsWith(`/${currentLocale}${item.href}/`)) ? "text-white bg-fibem-accent shadow-md" : ""
                    )}
                      ...
                      {item.submenu.map((subItem) => (
                        <Link
                          ...
                          className={cn(
                            ...
                            pathname === `/${currentLocale}${subItem.href}` || pathname?.startsWith(`/${currentLocale}${subItem.href}/`) ? "text-white bg-fibem-accent/80" : ""
                          )}
